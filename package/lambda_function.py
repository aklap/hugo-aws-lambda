...
def site_gen(event):
    ...
    check_hugo(input_bucket, TMP_DIR)
    check_content(input_bucket, TMP_DIR)
    run_hugo()
    upload_website(dst_bucket, PUB_DIR)


def check_hugo(input_bucket, tmp_dir):
...
def check_content(input_bucket, tmp_dir):
    ...
    try:
        ...
        print("Missing folder 'content' with at least 1 file!")
        raise RuntimeError("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output.decode('utf-8')))
